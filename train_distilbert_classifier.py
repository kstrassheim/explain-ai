...
from classes.ml import HuggingFacePretrainedClassifier
from classes.transformation import IdentityTransformation
from classes.preprocessing import IdentityPreprocessing, MaxLengthPreprocessing

class DistilbertClassifierTraining(AbstractMLTraining):

    def __init__(self, force_cpu = True, verbose=True, epochs=1):
        super(DistilbertClassifierTraining, self).__init__(force_cpu=force_cpu,verbose=verbose,epochs=epochs)
    ...
    def get_new_classifier_instance(self):
        return HuggingFacePretrainedClassifier(
                    pretrained_model_path='pretrained/distilbert-base-uncased',
                    max_input_len=500,
                    predict_classes_num = 2,
                    preprocessing = MaxLengthPreprocessing(max_text_length=500),
...
if __name__ == "__main__":
    ...
    training = DistilbertClassifierTraining(force_cpu=FORCE_CPU, verbose=VERBOSE, epochs=1
    ) 
